...
- (void)registerService {
	...
	[center requestAuthorizationWithOptions:(UNAuthorizationOptionBadge | UNAuthorizationOptionSound | UNAuthorizationOptionAlert) completionHandler:^(BOOL granted, NSError* _Nullable error) {
		if (!error) {
			NSLog(@"reqest authorization succeeded!");
		}
	}];
}
...
- (void)sendNotification:(Notification)notification {
	...
	UNNotificationRequest *reqest = [UNNotificationRequest requestWithIdentifier:@"Notification" content:content trigger:trigger];
	UNUserNotificationCenter *center = [UNUserNotificationCenter currentNotificationCenter];
	[center addNotificationRequest:reqest withCompletionHandler:nil];
}
